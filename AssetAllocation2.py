...
class DecisionLogger:
    ...
    def print_summary(self):
        ...
        for i, decision in enumerate(self.decisions):
            print(f"步骤 {i+1}: \t 状态: {decision['state']} \t 动作: {decision['action']} \t 奖励: {decision['reward']} \t 下一状态: {decision['next_state']}")
...
class PortfolioInvestmentEnv(gym.Env):
  ...
  def step(self, action):
      ...
      new_wealth = self.current_wealth * risk_allocation * (1+risk_return) + self.current_wealth * (1-risk_allocation) * (1+self.riskless_return)
...
class QLearningAgent:
  def __init__(
    ...
    #       (len(self.wealth_bins), 10)  # 财富状态 x 动作离散
    #   )
  
#   def discretize_state(self, state):
#       """将连续状态离散化"""
#       return np.digitize(state[0], self.wealth_bins) - 1
  ...
  def _get_best_action(self, state_key):
      ...
      state_actions = {
          ...
          for action in np.linspace(0, 1, 11)
      }
...
class SARSAAgent:
    ...
    def learn(self, state, action, reward, next_state, next_action, done):
        ...
        # 精确匹配动作
        action = round(float(action), 1)
        next_action = round(float(next_action), 1)
